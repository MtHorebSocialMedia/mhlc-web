...
async function clearCachedContent(contentId) {
    if (contentId) {
        await removeCacheEntry(`entry-${contentId.id}`);
...
async function getContentSection(sectionId) {
    const item = await getEntry('v2ContentSection', sectionId);
    return {
        id: item.sys.id,
